...
export async function createChatWorkspace(repo, branch) {
  ...
  const repoName = repo.split('/').pop() || 'code';
  const featureBranch = `${repoName}/new-chat-${shortId}`;
...
export async function getChatSettings() {
  ...
  try {
    ...
    const activeProvider = getConfigValue('LLM_PROVIDER') || process.env.LLM_PROVIDER || 'openrouter';
    ...
    const agentBackend = getConfigValue('AGENT_BACKEND') || process.env.AGENT_BACKEND || 'pi';
 ...
 * Uses a static list of known names — no read permissions needed.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Settings — GitHub Repo Management (security-token gated)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Get current GitHub repo config for the UI.
 * Returns owner, repo, whether GH_TOKEN is set, and whether security token is configured.
 */
export async function getGitHubRepoConfig() {
  await requireAuth();
  try {
    const { getSecretStatus } = await import('../db/config.js');
    const owner = process.env.GH_OWNER || '';
    const repo = process.env.GH_REPO || '';
    const tokenStatus = getSecretStatus(['GH_TOKEN']);
    const isTokenSet = tokenStatus.find((s) => s.key === 'GH_TOKEN')?.isSet || false;
    const securityTokenConfigured = !!process.env.ADMIN_SECURITY_TOKEN;
    return { owner, repo, isTokenSet, securityTokenConfigured };
  } catch (err) {
    console.error('Failed to get GitHub repo config:', err);
    return { error: 'Failed to load config' };
  }
}

/**
 * Update GitHub repo config (owner, repo, PAT). Requires ADMIN_SECURITY_TOKEN.
 * Writes GH_OWNER/GH_REPO to .env and process.env; stores GH_TOKEN in DB.
 */
export async function updateGitHubRepoConfig(securityToken, owner, repo, ghToken) {
  const user = await requireAuth();
  try {
    // Validate security token
    const expected = process.env.ADMIN_SECURITY_TOKEN;
    if (!expected) {
      return { error: 'ADMIN_SECURITY_TOKEN is not configured in .env' };
    }
    ...
    // Write GH_OWNER and GH_REPO to .env file
    ...
    // Update or append GH_OWNER
    const ownerRegex = /^GH_OWNER=.*$/m;
    if (ownerRegex.test(content)) {
      content = content.replace(ownerRegex, `GH_OWNER=${owner}`);
    } else {
      content = content.trimEnd() + `\nGH_OWNER=${owner}\n`;
    }

    // Update or append GH_REPO
    const repoRegex = /^GH_REPO=.*$/m;
    if (repoRegex.test(content)) {
      content = content.replace(repoRegex, `GH_REPO=${repo}`);
    } else {
      content = content.trimEnd() + `\nGH_REPO=${repo}\n`;
    }
    ...
    // Update process.env in memory so all reads pick up immediately
    process.env.GH_OWNER = owner;
    process.env.GH_REPO = repo;

    // Store GH_TOKEN in encrypted DB if provided
    if (ghToken) {
      const { setConfigSecret } = await import('../db/config.js');
      const { invalidateConfigCache } = await import('../config.js');
      setConfigSecret('GH_TOKEN', ghToken, user.id);
      invalidateConfigCache();
    }

    return { success: true };
  } catch (err) {
    console.error('Failed to update GitHub repo config:', err);
    return { error: err.message };
  }
}
