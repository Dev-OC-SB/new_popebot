 ...
 * @param {string} [options.codingAgent='pi'] - Coding agent identifier
 * @param {string} [options.id] - Optional ID (UUID). Generated if not provided.
 * @returns {object} The created workspace
 */
export function createCodeWorkspace(userId, { containerName = null, repo = null, branch = null, title = 'Code Workspace', codingAgent = 'pi', id = null } = {}) {
  const db = getDb();
  const now = Date.now();
  const workspace = {
    id: id || randomUUID(),
