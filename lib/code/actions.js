...
/**
 * Resolve the workspace home directory inside a Docker container.
 * @param {string} [codingAgent='pi'] - 'claude-code' or 'pi'
 * @returns {string}
 */
function workspaceHome(codingAgent) {
...
export async function startInteractiveMode(id) {
  ...
  try {
    ...
    await runCodeWorkspaceContainer({
      ...
      repo: workspace.repo,
      branch: workspace.branch,
      codingAgent: workspace.codingAgent || 'pi',
      featureBranch: workspace.featureBranch,
      workspaceId: id,
      chatContext,
    });
...
export async function getContainerGitStatus(id) {
  ...
  try {
    ...
    const home = workspaceHome(workspace.codingAgent);
    const [statusOut, headOut, logOut, unpushedOut] = await Promise.all([
      execInContainer(workspace.containerName, `cd ${home} && git status --short 2>/dev/null`),
      execInContainer(workspace.containerName, `cd ${home} && git rev-parse HEAD 2>/dev/null`),
      execInContainer(workspace.containerName, `cd ${home} && git log --format="- %h %s" ${contextRef}..HEAD 2>/dev/null`),
      execInContainer(workspace.containerName, `cd ${home} && git log --oneline @{u}..HEAD 2>/dev/null`).catch(() => null),
    ]);
...
export async function createTerminalSession(id, type = 'shell') {
  ...
  try {
    ...
    const home = workspaceHome(workspace.codingAgent);
    const codeCmd = workspace.codingAgent === 'claude-code'
      ? 'claude --dangerously-skip-permissions'
      : 'pi';
    const command = type === 'code'
      ? `nohup ttyd --writable -p ${port} bash -c 'cd ${home} && exec ${codeCmd}' > /dev/null 2>&1 &`
      : `nohup ttyd --writable -p ${port} bash -c 'cd ${home} && exec bash' > /dev/null 2>&1 &`;
