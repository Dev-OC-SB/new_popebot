...
function buildTemplateVars(cluster, role, workerUuid, payload) {
  ...
  const CLUSTER_HOME = agentBackend === 'claude-code' ? '/home/claude-code/workspace' : '/home/agent/workspace';
...
export async function runClusterRole(roleData, payload, trigger) {
  ...
  const containerHome = agentBackend === 'claude-code' ? '/home/claude-code/workspace' : '/home/agent/workspace';
  const binds = [`${hostDataDir}:${containerHome}`];
  ...
  try {
    const containerWorkDir = workerWorkDir.replace(dataDir, containerHome);
