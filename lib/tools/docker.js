 ...
 * @param {string} [options.codingAgent] - Coding agent identifier (defaults to AGENT_BACKEND config, or 'pi')
 * @param {string} [options.featureBranch] - Feature branch to create after cloning
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runCodeWorkspaceContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, chatContext }) {
  const agentBackend = codingAgent || getConfig('AGENT_BACKEND') || 'pi';
  const version = process.env.THEPOPEBOT_VERSION;

  let image;
  let homeDir;
  if (agentBackend === 'claude-code') {
    image = `stephengpope/thepopebot:claude-code-workspace-${version}`;
    homeDir = '/home/claude-code/workspace';
  } else {
    image = `stephengpope/thepopebot:pi-coding-agent-workspace-${version}`;
    homeDir = '/home/agent/workspace';
  }
  ...
  if (agentBackend === 'claude-code') {
    ...
    // Pass LLM provider config for Pi agent
    const llmProvider = getConfig('LLM_PROVIDER') || 'openrouter';
    const llmModel = getConfig('LLM_MODEL');
    env.push(`LLM_PROVIDER=${llmProvider}`);
    if (llmModel) env.push(`LLM_MODEL=${llmModel}`);
    const openrouterKey = getConfig('OPENROUTER_API_KEY');
    if (openrouterKey) env.push(`OPENROUTER_API_KEY=${openrouterKey}`);
    const anthropicKey = getConfig('ANTHROPIC_API_KEY');
    if (anthropicKey) env.push(`ANTHROPIC_API_KEY=${anthropicKey}`);
    ...
    if (googleKey) env.push(`GOOGLE_API_KEY=${googleKey}`);
    const baseUrl = getConfig('OPENAI_BASE_URL');
    if (baseUrl) env.push(`OPENAI_BASE_URL=${baseUrl}`);
  }
  ...
  const hostConfig = {};
  if (workspaceId) {
    hostConfig.Binds = [`${volumeName(workspaceId)}:${homeDir}`];
  }
...
async function runHeadlessCodeContainer({ containerName, repo, branch, featureBranch, workspaceId, taskPrompt }) {
  const agentBackend = getConfig('AGENT_BACKEND') || 'pi';
  const version = process.env.THEPOPEBOT_VERSION;

  let image;
  let homeDir;
  if (agentBackend === 'claude-code') {
    image = `stephengpope/thepopebot:claude-code-headless-${version}`;
    homeDir = '/home/claude-code/workspace';
  } else {
    image = `stephengpope/thepopebot:pi-coding-agent-headless-${version}`;
    homeDir = '/home/agent/workspace';
  }
  ...
  return runContainer({
    ...
    env,
    hostConfig: {
      Binds: [`${volumeName(workspaceId)}:${homeDir}`],
    },
  });
}
...
async function runClusterWorkerContainer({ containerName, image, env = [], binds = [], workingDir }) {
  ...
  if (image) {
    resolvedImage = image;
  } else if (agentBackend === 'claude-code') {
    resolvedImage = `stephengpope/thepopebot:claude-code-cluster-worker-${version}`;
  } else {
    resolvedImage = `stephengpope/thepopebot:pi-coding-agent-cluster-worker-${version}`;
  }
