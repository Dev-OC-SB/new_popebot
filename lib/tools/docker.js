...
async function runCodeWorkspaceContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, chatContext }) {
  ...
  const imageRepo = process.env.DOCKER_IMAGE_REPO || 'stephengpope/thepopebot';

  let image;
  let homeDir;
  if (agentBackend === 'claude-code') {
    image = `${imageRepo}:claude-code-workspace-${version}`;
    homeDir = '/home/claude-code/workspace';
  } else {
    image = `${imageRepo}:pi-coding-agent-workspace-${version}`;
...
async function runHeadlessCodeContainer({ containerName, repo, branch, featureBranch, workspaceId, taskPrompt }) {
  ...
  let homeDir;
  if (agentBackend === 'claude-code') {
    image = `${imageRepo}:claude-code-headless-${version}`;
    homeDir = '/home/claude-code/workspace';
  } else {
    image = `${imageRepo}:pi-coding-agent-headless-${version}`;
    homeDir = '/home/agent/workspace';
  }
...
async function runClusterWorkerContainer({ containerName, image, env = [], binds = [], workingDir }) {
  ...
  if (image) {
    ...
    resolvedImage = `${imageRepo}:claude-code-cluster-worker-${version}`;
  } else {
    resolvedImage = `${imageRepo}:pi-coding-agent-cluster-worker-${version}`;
  }
