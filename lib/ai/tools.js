  ...
  return tool(
    async ({ task_description }) => {
      try {
        ...
        const codingAgent = workspace?.codingAgent || 'pi';
  ...
  return tool(
    async ({ task_description }) => {
      try {
        ...
        const featureBranch = workspace?.featureBranch || `${repoName}/new-chat-${workspaceId.replace(/-/g, '').slice(0, 8)}`;
    ...
    },
    {
      ...
      description:
        'Launch a headless coding task in a Docker container. The task runs a coding agent, commits changes, and merges back automatically. Returns immediately after launching — output streams live in the chat. Use this as the default when the user wants to implement something.',
      schema: z.object({
  ...
  return tool(
    async ({ file_path }) => {
      try {
        ...
        if (is404) {
          return `File "${file_path}" does not exist on branch "${branch}". This is expected if the file has not been created yet or if the coding session has not pushed to this branch. Do NOT retry this request — the file is not available.`;
        }
        return `Error fetching ${file_path}: ${err.message || 'Unknown error'}`;
      }
    },
    {
