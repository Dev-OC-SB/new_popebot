  ...
  if (options.repo && options.branch) {
    ...
    if (!existingChat) {
      ...
      if (!workspaceId) {
        ...
        const repoName = (options.repo || '').split('/').pop() || 'code';
  ...
  try {
    ...
    if (response.title.trim()) {
      ...
      if (chat.codeWorkspaceId) {
        try {
          const { updateFeatureBranch, getCodeWorkspaceById } = await import('../db/code-workspaces.js');
          const ws = getCodeWorkspaceById(chat.codeWorkspaceId);
          const repoName = (ws?.repo || '').split('/').pop() || 'code';
          ...
          const featureBranch = `${repoName}/${slug}-${shortId}`;
